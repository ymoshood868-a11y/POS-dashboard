...
async function loadForEdit(id) {
  try {
    ...
    if (pageSubtitle)
      pageSubtitle.textContent = `Editing transaction ${txn.reference || "#" + id}.`;
  ...
  try {
    ...
    const msg =
      err.status === 404
        ...
        : `Failed to save transaction: ${err.message || "Make sure the Express server is running (cd server && npm start)."}`;
...
function validateForm() {
  ...
  if (!rawAmount || isNaN(amount) || amount <= 0) {
    showFieldError(
      "group-amount",
      fldAmount,
      "Amount must be a positive number (e.g. 150 or 99.99).",
    );
    valid = false;
    ...
    showFieldError(
      "group-amount",
      fldAmount,
      "Amount accepts up to 2 decimal places only.",
    );
    valid = false;
  }
...
function updatePreview() {
  ...
  // Type
  previewType.textContent = fldType.value ? capitalize(fldType.value) : "—";
  ...
  previewAmount.textContent =
    !isNaN(amount) && amount > 0 ? formatCurrency(amount) : "$0.00";

  // Date
  previewDate.textContent = fldDate.value ? formatDate(fldDate.value) : "—";
...
function showFieldError(groupId, inputEl, message) {
  ...
  inputEl.addEventListener("input", () => clearFieldError(groupId), {
    once: true,
  });
  inputEl.addEventListener("change", () => clearFieldError(groupId), {
    once: true,
  });
}
